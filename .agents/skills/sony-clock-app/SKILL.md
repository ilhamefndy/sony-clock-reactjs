...
description: >
  ...
  implements the official 3-column schedule format (Time IN | Half Day | Time OUT),
  ...
  19:00 (7:00 PM) max standard clock-out capping, early clock-in detection (before 07:30 AM),
...
| **Official 3-Column Schedule Format** | Prominently displays `Time IN` → `Half Day (+4.75h)` → `Time OUT (+9.5h)` matching company schedule chart |
| **Official Lookup Matrix** | Interactive 1-min & 5-min lookup table matching official company table format with active row highlighting |
